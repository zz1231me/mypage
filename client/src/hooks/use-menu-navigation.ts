...
/**
 * Hook that implements keyboard navigation for dropdown menus and toolbars.
...
export function useMenuNavigation<T>({
  ...
  useEffect(() => {
    ...
    const targetElement = containerRef?.current ?? null;
    if (targetElement) {
    ...
    return undefined;
  }, [containerRef, items, selectedIndex, onSelect, onClose, orientation]);
